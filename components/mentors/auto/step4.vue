    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-[1fr,360px] gap-6 lg:gap-8">
      ...
      <main>
        ...
        <section class="mt-7">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <span class="text-[16px] text-[#101012]" v-else>{{ m.label }}</span>
                <span
                  class="ml-auto radio-outer"
                  :class="payMethod === m.id ? 'on' : 'off'"
                >
              ...
              <input
                ...
                @change="payMethod = m.id"
              />
            </label>

            <!-- “Списать с баланса” как отдельная строка -->
            <label class="sm:col-span-2 pay-card">
              <div class="flex items-center gap-3">
                <span class="text-[16px] leading-[24px] tracking-[-0.25px] text-[#101012]">
                  Списать с баланса
                </span>
          ...
          <!-- Привязка карты -->
          <div
            class="mt-4 rounded-[16px] bg-[#E8F1FF] border border-[#CFE3FF]
                   p-4 sm:p-5"
          >
            <label class="flex items-center gap-3 cursor-pointer select-none">
              <button
                type="button"
                class="switch"
                :class="bindCard ? 'switch--on' : ''"
                @click="bindCard = !bindCard"
              >
                <span class="switch__dot"></span>
              </button>
              <div>
                <div
                  class="text-[16px] leading-[24px] tracking-[-0.25px]
                         text-[#101012] font-medium"
                >
                  Привязать карту для будущих звонков
                </div>
                <div
                  class="text-[14px] leading-[20px] text-[rgba(60,60,67,0.72)]"
                >
        ...
        <label class="mt-6 flex items-start gap-3">
          <input
            type="checkbox"
            v-model="agree"
            class="mt-[3px] h-4 w-4 rounded border border-[#D0D0D0]"
          />
      ...
      <aside>
        <div class="sticky top-4">
          <div
            class="rounded-[20px] border border-[#E6E6E6] bg-white overflow-hidden
                   shadow-[0_1px_0_rgba(0,0,0,0.02)]"
          >
            <div
              class="px-4 sm:px-5 py-4 border-b border-[#E6E6E6]
            ...
            <div class="px-4 sm:px-5 py-4 space-y-4">
              <div class="row">
                <div class="row__l">
                  <div class="text-[14px] text-[rgba(60,60,67,0.72)]">
                    Стоимость ментора
                  </div>
                  <div class="text-[14px] text-[#101012] font-medium">
                    {{ mentorName || 'Имя ментора' }}
                  </div>
                </div>
                <div class="row__r text-right">
                  <div
                    v-if="oldPrice && oldPrice > pricePer"
                    class="text-[13px] line-through text-[#3C3C43]/50"
                  >
                    {{ fmt(oldPrice) }}
                  </div>
                  <div class="text-[16px] leading-[20px] tabular-nums">
                    {{ fmt(pricePer) }}
                  </div>
                </div>
              </div>
              ...
              <div class="row">
                <div class="row__l text-[14px] text-[#101012]">
                  Количество звонков
                </div>
                <div class="row__r">
                  <div
                  ...
                  >
                    <button class="h-8 w-8" @click="decQty" :disabled="qty <= 1">
                      −
                    </button>
                    <div class="h-8 w-8 grid place-items-center tabular-nums">
                      {{ qty }}
                    </div>
                    <button class="h-8 w-8" @click="incQty" :disabled="qty >= 16">
                      +
                    </button>
                  </div>
                </div>
              ...
              <div class="row">
                ...
                <div class="row__r text-[14px] text-[#007AFF]">
                  −{{ fmt(discount) }}
                </div>
              </div>
              ...
              <div>
                <button
                  class="text-[14px] text-[#007AFF] underline underline-offset-2"
                ...
                <transition name="fade-slide">
                  <div v-if="couponOpen" class="mt-2 flex gap-2">
                    <input
                      v-model="couponCode"
                      placeholder="Код"
                      class="h-9 flex-1 rounded-[10px] border px-3 text-[14px]
                             [border-color:rgba(120,120,128,0.16)]"
                    />
                    <button
                      class="h-9 px-3 rounded-[10px] bg-[#F2F3F5]"
                      @click="applyCoupon"
                    >
                      Применить
                    </button>
            ...
            <div class="px-4 sm:px-5 py-4 border-t border-[#E6E6E6]">
              <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                  <div
                    v-if="hasDiscount"
                    class="text-[12px] text-[#3C3C43]/60 line-through tabular-nums"
                  >
                    {{ fmt(totalBefore) }}
                  </div>
                  <div
                    class="text-[24px] leading-[24px] font-medium tabular-nums"
                  >
                    {{ fmt(total) }}
                  </div>
                </div>
...
let timer: any = null
const endAt = Date.now() + props.timerMs
...
onMounted(() => {
  timer = setInterval(() => {
    ...
    progressPct.value = Math.max(0, (left / props.timerMs) * 100)
...
/* оплата */
const payMethod = ref<'yo' | 'stripe' | 'paypal' | 'crypto'>('yo')
const payMethods = [
...
const couponApplied = ref(props.couponDefault)

const subtotal = computed(() => (props.pricePer ?? 0) * (qty.value ?? 1))
const serviceFee = computed(() => Math.round(subtotal.value * (props.feeRate ?? 0)))
const totalBefore = computed(() => subtotal.value + serviceFee.value)
const total = computed(() =>
  Math.max(totalBefore.value - discount.value - couponApplied.value, 0)
)
...
function applyCoupon() {
  couponApplied.value = couponCode.value.trim().toUpperCase() === 'GIFT2000' ? 2000 : 0
}
...
/* switch */
.switch{
  width:44px; height:26px; border-radius:9999px; background:#DADDE2;
