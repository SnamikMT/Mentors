...
<template>
  <NuxtLayout name="app">
    <!-- Десктопный Header -->
    <Header class="hidden lg:block mb-8" sectionTitle="Карьерный путь" :sectionIcon="sectionIcon" />
    ...
    <div
      class="grid grid-cols-1 items-start
             lg:grid-cols-[600px,360px] lg:gap-12
             2xl:grid-cols-[720px,432px]
             3xl:grid-cols-[840px,504px]"
    >
      <!-- LEFT -->
      <section class="min-w-0 w-full lg:[grid-column:1]">
        <!-- Бейдж -->
        <div class="mb-6">
          <span
      ...
      <!-- RIGHT -->
      <aside
        class="min-w-0 w-full bg-white rounded-[32px] border border-black/10 overflow-hidden
               lg:[grid-column:2]"
      >
        <!-- Цель -->
        <section class="p-5 sm:p-6 max-w-full">
          <div class="text-[#020203] font-medium mb-2" style="font-size:20px; line-height:20px; letter-spacing:-0.33px">
            Цель
          </div>
        ...
        <section class="p-5 sm:p-6 max-w-full">
          <div class="mb-3">
            <svg viewBox="-28 -28 56 56" class="w-16 h-16 sm:w-[90px] sm:h-[90px] max-w-full">
              <g>
...
<style scoped>
/* защита от «распирания» контента */
:deep(img),
:deep(video),
:deep(svg),
:deep(canvas) { max-width: 100%; height: auto; }
</style>
