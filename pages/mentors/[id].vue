  ...
  <div class="w-full max-w-[1440px] bg-Backgrounds-Neutral-Primary flex flex-col lg:flex-row gap-0 lg:gap-12 overflow-hidden">
    ...
    <div class="w-full lg:max-w-[1008px] px-4 sm:px-6 lg:pl-[48px] lg:pr-6 pt-[56px] lg:pt-6 flex flex-col">
      ...
      <div class="hidden lg:block mb-4">
        <button
          type="button"
          class="inline-flex items-center gap-2 h-10 px-3 rounded-[12px] border bg-transparent
                 [border-color:rgba(120,120,128,0.16)]
                 text-[16px] leading-[24px] text-[#101012] hover:bg-black/[0.02] transition"
          @click="goBack"
        >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[320px,1fr] gap-6 md:gap-8 lg:gap-9">
        ...
        <aside class="flex flex-col gap-4 overflow-visible">
          ...
          <div class="flex flex-col space-y-3">
            <button class="preview-card"><img :src="works[0]" class="w-full h-full object-cover" alt=""></button>
            <button class="preview-card"><img :src="works[1]" class="w-full h-full object-cover" alt=""></button>
            <button class="preview-card"><img :src="works[2]" class="w-full h-full object-cover" alt=""></button>
          </div>
        </aside>
        ...
        <main class="flex flex-col">
          ...
          <!-- Подсказка с иллюстрацией -->
          <section class="py-7 md:py-9">
            <div
              class="w-full lg:w-[652px] rounded-[16px] border border-black/10 bg-white p-4 sm:p-6
                     grid grid-cols-1 sm:grid-cols-[1fr,140px] gap-4 items-center"
            >
              <div class="flex flex-col gap-3">
                <div class="text-[#101012] font-medium text-[20px] sm:text-[24px] leading-[24px] [letter-spacing:-0.5px]">
                  Не знаешь, какого<br class="hidden sm:block" />
                  ментора лучше выбрать?
                </div>
                <button
                  type="button"
                  class="inline-flex items-center justify-center h-10 w-full sm:w-[226px] px-4 rounded-full
...
const aboutFirst = `Knitted knitted bludger owl bludger cabbage in horseless...`
const aboutHidden = `Full-moon bezoar forbidden carriages petrified snape...`
const mentorFirst = `Knitted knitted bludger owl bludger cabbage in horseless...`
const mentorHidden = `Full-moon bezoar forbidden carriages petrified snape...`

const onBooked = (payload) => { console.log('Booked:', payload); isModalOpen.value = false }
</script>
...
.btn-primary{
  ...
  height: 56px; width: 100%; border-radius: 20px; color: #fff; font-weight: 500; background: #007AFF; overflow: hidden;
}
.btn-primary::after{ content:""; position:absolute; inset:0; background:linear-gradient(180deg,rgba(255,255,255,.22) 0%,rgba(255,255,255,0) 45%); }
...
/* типографика */
.subheading{ @apply text-[22px] leading-[24px] md:text-[24px] md:leading-[24px] tracking-[-0.5px] font-medium; }
...
.preview-card:hover{ transform: scale(1.04); box-shadow: 0 28px 80px rgba(0,0,0,0.28); }
