    ...
    <main class="flex flex-col-reverse lg:flex-row flex-1 relative">
      ...
      <section class="w-full lg:w-1/2 px-6 sm:px-12 lg:px-32 py-10 flex flex-col">
        <h1 class="text-[32px] sm:text-[40px] lg:text-[48px] font-medium leading-tight text-black">
          Вход в Менторс<br />
          <span class="text-[#3C3C43]/40">для бизнеса</span>
        </h1>
        ...
        <!-- SSO -->
        <button
          class="mt-3 py-4 w-full max-w-md bg-white rounded-2xl border border-gray-200 flex justify-center items-center gap-2 hover:shadow-sm"
        >
        ...
        <!-- Без аккаунта -->
        <button class="mt-3 py-4 w-full max-w-md bg-[#787880]/10 rounded-2xl flex justify-center items-center gap-2 hover:bg-[#787880]/15">
          <span class="text-base font-medium text-black">У меня нет аккаунта</span>
        </button>
      </section>
...
import loginImage from '../src/assets/img/login-page/image4.png'
...
const goToPin = () => {
  if (mail.value.trim()) router.push('/login/business/pin')
