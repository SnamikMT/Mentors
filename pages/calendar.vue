<template>
  <!-- ✅ Мобильный столбец, десктоп — две колонки -->
  <div
    class="w-full max-w-[1440px] bg-Backgrounds-Neutral-Primary flex flex-col lg:flex-row items-stretch gap-0 lg:gap-12 overflow-hidden relative"
  >
    <!-- Sidebar: только ≥lg -->
    <Sidebar class="hidden lg:flex" />
    <!-- Mobile header: только <lg (сверху, над всем) -->
    <MobileHeader class="lg:hidden" />

    <!-- Контент -->
    <div
      class="w-full lg:max-w-[1008px] pt-4 lg:pt-[16px] px-4 sm:px-6 lg:px-[64px] flex flex-col relative"
    >
      ...
      <div class="mt-8 lg:mt-[64px] flex flex-col sm:flex-row gap-3">
        <button
          class="flex items-center justify-center gap-2 h-12 px-4 bg-gradient-to-b from-[#2F80ED] to-[#1C6DD0] rounded-2xl text-white font-medium"
        >
          <img :src="joinCallIcon" class="w-6 h-6" alt="Join Icon" />
          Зайти в звонок
        </button>
        <button
          class="h-12 px-4 border rounded-2xl border-[#7878801a] text-[#020203] font-medium flex items-center justify-center gap-2"
        >
      ...
      <!-- десктоп-поповер (не влияет на поток) -->
      <Calendar
      ...
      <!-- Прогресс + карточки -->
      <div class="flex flex-col lg:flex-row gap-3 lg:gap-[12px] mt-20 lg:mt-[164px]">
        <!-- Первая карточка -->
        <div
        ...
        >
          <div class="flex flex-col justify-between">
            ...
            <button
              class="mt-4 px-4 py-2 text-[#020203] bg-[#F2F2F7] rounded-full font-medium w-max text-sm"
            >
              Читать гайд
            </button>
          </div>
